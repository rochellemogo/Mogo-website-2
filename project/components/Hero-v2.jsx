const HeroV2 = () => {
  ...
  const badge    = s.hero_badge    || 'Productivity financing · 100,000+ Kenyans backed · 84 branches';
  const headline = s.hero_headline || 'Finance your future.';
  const tagline  = s.hero_tagline  || 'Jenga kesho yako.';
  const body     = s.hero_body     || 'We back the bike, the car, the phone, the shop — the tools Kenyans actually earn with.';

  return (
    <section data-mogo-hero style={{ position: 'relative', overflow: 'hidden', marginTop: -80, paddingTop: 80, background: 'var(--m-ink)', color: '#fff' }}>
      <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
        {s.hero_image
          ? <img src={s.hero_image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      ...
      <div className="shell" style={{ position: 'relative', zIndex: 2, padding: '120px 28px 180px' }}>
        ...
        <div data-hero-jenga style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: 'clamp(28px, 3.2vw, 52px)', color: 'rgba(255,255,255,.85)', margin: '18px 0 0', letterSpacing: '-.01em' }}>
          {tagline}
        </div>

        <p data-hero-blurb style={{ fontSize: 20, lineHeight: 1.5, color: 'rgba(255,255,255,.78)', maxWidth: 620, margin: '36px 0 40px' }}>{body}</p>
...
const InvestorStripV2 = () => {
  ...
  const logos = (s.partners && s.partners.length) ? s.partners : ['Boxer Motors', 'TVS Kenya', 'Bajaj', 'Honda', 'Tecno', 'Infinix', 'Itel', 'Samsung'];
